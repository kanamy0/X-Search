...
- **自然文フィルタ**: 「フォロワー1000人以上」「投稿が多い人」「フォロー500以下」などの自然文でフォロワー数・フォロー数・投稿数を指定し、候補を **Gemini 分析の前に** 絞り込み(API 呼び出し回数を削減)。
- **性別による絞り込み(推定)**: 性別は厳密には不明なため、Gemini が公開プロフィール・投稿の文体や話題から性別(男性/女性/不明・確信度付き)を推定。UI で性別を選択すると推定性別で候補を絞り込む(推定のため分析後に適用)。
- **X API 検索**: 公開投稿を recent search で検索し、投稿・ユーザーを収集。
- **ユーザー / 投稿取得**: 公開プロフィールと最新投稿(約 20 件)を取得し SQLite に保存(重複取得は回避)。
- **Gemini 分析**: プロフィールと投稿をまとめて Google Gemini へ送信し、趣味・関心の特徴量を JSON で取得。軽量モデル(既定 `gemini-2.5-flash-lite`)と投稿本文のトリムでトークンを節約。
- **推薦ロジック**: 入力キーワードから理想の特徴量ベクトルを生成し、コサイン類似度で一致率 (0〜100%) を算出。
- **キャッシュ**: 分析済みユーザーは再分析せず、更新日時が 7 日以上前の場合のみ再分析。入力キーワードの分析結果も同一入力ならキャッシュを再利用。
...
2. 任意で「絞り込み条件」に自然文を入力(例: `フォロワー1000人以上、投稿が多い人`)。
3. 任意で「性別」を選択(指定なし/男性/女性)。投稿から推定した性別で絞り込みます。
4. 「検索」ボタンを押す。
5. 右側におすすめアカウントがカード表示されます。「詳細」で詳細画面へ。
...
│   ├── analysis_service.py    # 検索→絞り込み→分析→推薦のオーケストレーション
│   ├── filter_service.py      # 自然文の絞り込み条件パーサ (ルールベース)
│   ├── recommend_service.py   # コサイン類似度による一致率計算
│   └── schemas.py             # 特徴量プロファイル / フィルタの pydantic スキーマ
├── ui/
